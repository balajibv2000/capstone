...
PROCESS_THREAD(udp_client_process, ev, data)
{
  ...
  while(1) {
    ...
    if(NETSTACK_ROUTING.node_is_reachable() &&
      ...
      LOG_INFO_("\n");
			int temp = (random_rand() % (40 - 32 + 1) + 32);
			int humidity = (random_rand() % (30 - 70 + 1) + 30);
			int uv = (random_rand() % (150 - 120 + 1) + 120);
			int pressure = (random_rand() % (720 - 820 + 1) + 720);
      sprintf(str, "%d %d %d %d;" , temp , humidity , uv , pressure );
